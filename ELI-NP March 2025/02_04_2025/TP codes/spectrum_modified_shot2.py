...
# %%
imagepath = r"D:\data Lab\ELI-NP March 2025\02_04_2025\separated TP images\02042025_ESM_5_TP_20_15-[Phosphor]_shot2.tif"
...
image = image[a:b,:]
description = r"Ni flat 2$\mu$m (2 shots integrated)"
...
Background = np.mean(trace[L_low:L_high,:],axis=0)
# Subtract Background 
trace = trace - Background

trace = np.where(trace < 1, 1, trace)   # make the negative values positive (and >1, otherwise log scale plot makes issue) 

plt.imshow(np.log(trace),cmap="jet",aspect=0.1)
plt.axhline(y=L_low, lw=2, color="w")
plt.axhline(y=L_high, lw=2, color="w")
...
cutoff_index_integrated_count = find_index(integrated_count,np.max(integrated_count)*0.99995)
...
boundary_energy_in_MeV1 = 12
boundary_energy_in_MeV2 = 15
...
ax1.set_xlim(0, 50)
minw = find_index(Energy,40*keV_to_Joule*1e3)
maxw = find_index(Energy,50*keV_to_Joule*1e3)
min_ylim = 0.5*np.min(counts[minw:maxw])
ax1.set_ylim(min_ylim,np.max(counts)*2)
...
np.savetxt(fr"{parent_dir}\separated TP images\{txtfilename}", data, fmt='%.5f', header='Energy(MeV) Flux(/MeV/Srad)', comments='')
