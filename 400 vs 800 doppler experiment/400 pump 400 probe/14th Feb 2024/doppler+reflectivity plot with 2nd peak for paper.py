...
maxw = find_index(delay_reflectivity,18)
t1 = delay_reflectivity[2:maxw]
sig1 = (norm_signal/np.max(norm_signal))[2+shift:maxw+shift]

minw1 = find_index(t1,11.8) 
maxw1 = find_index(t1, 15)

t1 = t1[minw1:maxw1]+0.2
sig1 = sig1[minw1:maxw1]

fit_sig1, parameters1 = eft.fit_exp_decay(t1, sig1)
...
line3, = ax2.plot(t,fit_sig,"k-",lw=5,label="Exponential fit")
line4, = ax2.plot(t1,fit_sig1,"-",lw=5,color="purple",label="Exponential fit")
...
class HandlerDoubleLine(HandlerBase):
    def create_artists(self, legend, orig_handle, x0, y0, width, height, fontsize, trans):
        # Line 1 (magenta)
        red_line = Line2D([x0, x0 + width * 0.4], [y0 + height / 2]*2,
                          color='k', linestyle='-', linewidth=5, transform=trans)
        # Line 2 (purple)
        blue_line = Line2D([x0 + width * 0.6, x0 + width], [y0 + height / 2]*2,
                           color='purple', linestyle='-', linewidth=5, transform=trans)
        return [red_line, blue_line]

# Dummy handle for the red+blue dots
double_dot = Line2D([], [], linestyle='None')
double_line = Line2D([], [], linestyle='None')

# Now create the legend with two entries
legend = ax1.legend(
    [double_dot, ax2.lines[0],double_line],   # <- double dot first, then cos(x) plot
    ['Doppler shift', 'Reflectivity', 'Exponential fits'],
    facecolor="lightgray", loc='lower right',
    frameon=True,     # Enable the frame
    edgecolor='black', # Frame color
    handler_map={double_dot: HandlerDoubleDot(),double_line:HandlerDoubleLine()}
)
