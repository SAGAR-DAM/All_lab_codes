...
files_4 = sorted(glob.glob("D:\\data Lab\\400 vs 800 doppler experiment\\400 pump 400 probe\\04 th april 2024\\pos_4\\scan 1\\*.txt"))
...
peaks = []
delay = np.linspace(12,15,len(files_4)//2)-12.65
delay = 2*delay/c
delay = np.around(delay, decimals=3)

for i in range(1,len(files_4),2):
    f = open(files_4[i])
    r=np.loadtxt(f,skiprows=17,comments='>')
...
plt.plot(delay[0:len(peaks)-1],peaks[0:len(peaks)-1]-411.0, 'ro')
plt.plot(delay[0:len(peaks)-1],peaks[0:len(peaks)-1]-411.0, 'k-')
plt.title("Doppler shift")
plt.xlabel("probe delay (ps)")
plt.ylabel("Peak wavelength (nm)")
# plt.xlim(-5,18)
# plt.ylim(-0.125,0.5)
plt.title("Doppler Shift for pos 4")
plt.grid(lw = 1, color = "black")
plt.show() 
...
files_4 = sorted(glob.glob("D:\\data Lab\\400 vs 800 doppler experiment\\400 pump 400 probe\\04 th april 2024\\pos_4\\scan 2\\*.txt"))
