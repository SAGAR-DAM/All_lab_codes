...
ax.fill_between(delay1, v1 - v1_lerr, v1 + v1_uerr, color="k", alpha=0.2)
...
ax.fill_between(delay2, v2 - v2_lerr, v2 + v2_uerr, color="k", alpha=0.2)
...
ax.set_title(
    ...
    fr"Intensity: 1.3$\times$" + r"$10^{18}$ W/cm$^2$" 
    # + "; (Red: inside the target)",
    ,fontweight='bold'
    ,fontsize=30
)
ax.set_xlabel("Probe delay (ps)", fontweight='bold')
ax.set_ylabel("velocity (cm/s)", fontweight='bold', color="brown")
...
class HandlerDoubleDiamond(HandlerBase):
    def create_artists(self, legend, orig_handle, x0, y0, width, height, fontsize, trans):
        red_diamond = Line2D([x0 + width * 0.3], [y0 + height/2],
                         marker='d', color='brown', linestyle='None',
                         markersize=10, transform=trans)
        blue_diamond = Line2D([x0 + width * 0.7], [y0 + height/2],
                          marker='d', color='blue', linestyle='None',
                          markersize=10, transform=trans)
        return [red_diamond, blue_diamond]

# Dummy handle for the red+blue dots
double_dot = Line2D([], [], linestyle='None')
double_diamond = Line2D([], [], linestyle='None')

# Now create the legend with two entries
legend = ax.legend(
    [double_dot,double_diamond, ax.lines[-1]],   # <- double dot first, then cos(x) plot
    ['Experiment Scan 1','Experiment Scan 2','Simulation'],
    facecolor="lightgray", loc='upper right',
    frameon=True,     # Enable the frame
    edgecolor='black', # Frame color
    handler_map={double_dot: HandlerDoubleDot(),double_diamond: HandlerDoubleDiamond()}
)
...
ab = AnnotationBbox(imbox, (6.8, 0.28e7),frameon=False, zorder=10)  # High z-order ensures it's in front
ax.add_artist(ab)
