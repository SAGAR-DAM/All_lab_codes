...
for file in files:
    ...
    t.append(float(ds.current_time)*1e12)
...
line1, = ax1.plot((delay[0:len(peaks)])[::1],peaks[::1]-395.3, 'ro',label="Doppler Shift",color="brown",markersize=10)
...
delta_lamda_from_sim = -2*412*vel_sim/c_in_cm_s
# line3, = ax2.plot(t[:40]-2.5,-delta_lamda_from_sim[:40],"m-",lw=5,label="Exponential fit")
...
# ab = AnnotationBbox(imbox, (12, 0.52e7),frameon=False, zorder=10)  # High z-order ensures it's in front
# ax.add_artist(ab)
