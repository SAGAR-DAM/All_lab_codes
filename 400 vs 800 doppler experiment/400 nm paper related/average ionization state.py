...
for i in range(len(files)):
    ...
    ye = np.array((ad[('flash', 'ye')])[:maxw])
    sumy = np.array((ad[('flash', 'sumy')])[:maxw])
    Z = ye/sumy
    ...
    Z_arr.append(np.sum(weighted_Z))
    
    # plt.plot(Z,label="Z")
    # plt.plot(weight*max(Z)/max(weight),label="weight from ion density")
    # plt.legend()
    # plt.show()

# %%

t = np.linspace(0,10.03,len(Z_arr))

plt.plot(t,Z_arr,"r-",lw=5,label=r"$\overline{Z(t)}$")
plt.title("Ionization state \n(Averaged over plasma length)",fontsize=23)
plt.xlabel("delay (ps)")
# plt.text(x=6,y=7.5,s=r"$\langle\overline{Z(t)}\rangle_t$: "+f"{np.mean(Z_arr):.2f}")
