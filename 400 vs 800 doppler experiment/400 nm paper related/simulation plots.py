...
for file in files:
    ...
    e_dens_in_10_e_21 = np.array(ad['gas', 'El_number_density'])
    
    x = np.array(ad['gas', 'x'])*1e4
    
    pos.append(x[index])

    axs[0].plot(x,e_dens_in_10_e_21,label=f"{time:.1f} ps",lw=2)
    
    


axs[0].axhline(6.97e21,linestyle="--",color="k",lw=2)
# axs[0].text(x=2.55,y=3e21,s="Critical Density line\n"+r"$n_c=6.97\times10^{21} cm^{-3}$")
axs[0].set_xlim(2,3)
axs[0].set_ylim(1e20,)
axs[0].set_yscale("log")
axs[0].set_xlabel(r"x ($\mu$m)")
axs[0].set_ylabel(r"n$_e$  (cm$^{-3}$)")
...
for file in files:
    ...
    density = np.array(ad["gas","density"])
    x = np.array(ad['gas', 'x'])*1e4
    
    pos.append(x[index])

    axs[1].plot(x,density,label=f"{time:.1f} ps",lw=2)
    
    


# axs[0].axhline(1,linestyle="--",color="k",lw=2)
# axs[0].text(x=2.51,y=0.72,s="Critical Density line\n"+r"$n_c=6.97\times10^{21} cm^{-3}$")
axs[1].set_xlim(2,3)
axs[1].set_xlabel(r"x ($\mu$m)")
axs[1].set_ylabel(r"$g\cdot cm^{-3}$")
# axs[0].ylim(-1,5)
# axs[0].legend()
...
for file in files:
    ...
    Te = np.array(ad["flash","tele"])/11604
    x = np.array(ad['gas', 'x'])*1e4
    
    pos.append(x[index])

    axs[2].plot(x,Te,label=f"{time:.1f} ps",lw=2)
...
# Get handles and labels from one axis (they are the same across all)
