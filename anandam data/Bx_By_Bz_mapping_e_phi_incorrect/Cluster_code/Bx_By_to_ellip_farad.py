...
Bx_By_to_ellip_farad_adaptive_step = np.vectorize(Bx_By_to_ellip_farad_adaptive_step)
...
def process_chunk(chunk):
    for Bz in chunk:
        B_max = 100
        resolution = 201
        Bx, By = np.mgrid[-B_max:B_max:resolution * 1j, -B_max:B_max:resolution * 1j]
...
if __name__ == "__main__":
    ...
    Bz_values = np.linspace(0, 100, 101)

    # Divide the Bz values into chunks for each core
    cpu_count = multiprocessing.cpu_count()
    chunk_size = len(Bz_values) // cpu_count
    chunks = [Bz_values[i:i + chunk_size] for i in range(0, len(Bz_values), chunk_size)]
