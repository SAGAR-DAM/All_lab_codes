...
a4_array=np.arange(3,5,0.1)*10**(-42)
...
for i in range(len(a4_array)):
    ...
    plt.grid()
    plt.xlabel("z position in mm"+"\n"+r"$\alpha_4: $"+f"{alpha4:.2f}"+r"$\times 10^{-32} cm^5W^{-3}$"+"        "+"absolute sq error: %f"%(absolute_error))
...
plt.plot(sample_pos,fit_y,'k-',label="Fit data")
plt.grid()
plt.xlabel("z position in mm"+"\n"+r"$\alpha_4: $"+f"{alpha4:.2f}"+r"$\times 10^{-32} cm^5W^{-3}$"+"        "+"absolute sq error: %f"%(min_abs_error))
plt.ylabel("Normalized transmission T(z)")
plt.title("4 Photon absorption by z-scan of ga2O3"+"\n"+filename_at_title)
plt.show()
