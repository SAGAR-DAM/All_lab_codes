...
for i in range(len(files)):
    ...
    plt.grid()
    plt.xlabel("z position in mm"+"\n"+r"$\alpha_4: $"+f"{alpha4:.2f}"+r"$^{+%.1f}$"%(a4_uerr[filenumber])+r"$_{-%.1f}$"%(a4_lerr[filenumber])+r"$^\times 10^{-32}\ cm^5W^{-3}$",fontsize = 12)
    plt.ylabel("Normalized transmission T(z)",fontsize = 12)
    plt.title("4 Photon absorption by z-scan of"+r" $\beta-Ga_2O_3$"+"\n"+f"Energy: {E[i]*1e6:.1f}"+r"$\pm$"+f"{E_err[i]*1e6:.2f} "+r"$\mu J$ ; "+f"Intensity: {I[i]*1e-4*1e-12:.2f}"+r"$\pm$"+f"{I_err[i]:.2f} "+r"$TW/cm^2$")
