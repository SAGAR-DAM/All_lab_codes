...
E_err=np.array([0.91,0.74,2.52])*1e-6
...
a4_array=np.array([4.1,3.5,4.2])*1e-42
a4_uerr = np.array([0.6,0.7,0.6])
a4_lerr = np.array([0.5,0.6,0.6])
...
def w(z):
    ...
    # value = w0*np.sqrt(1+((lamda*z)/(np.pi*w0**2))**2)
    return value
...
for i in range(len(files)):
    ...
    plt.plot(position[::5],x[::5],'o-',label=r"$I_0$= %1.2f"%(I[filenumber]*10**(-16))+r" $TW/cm^2$",markersize=3,linewidth=0.7)
    if(filenumber<len(files)-1):
        plt.plot(sample_pos,fit_y,'k-',linewidth=1.2)
    else:
        plt.plot(sample_pos,fit_y,'k-',label="Fit",linewidth=1.2)
        
    # plt.plot(sample_pos,fit_y,'k-',label="Fit data")
    plt.legend()
    plt.grid(color="k",lw=0.5)
    plt.xlabel("z position in mm",fontsize = 12)
    plt.ylabel("Normalized transmission T(z)",fontsize = 12)
    plt.title("4 Photon absorption by z-scan of"+r" $\beta-Ga_2O_3$")
