...
plt.plot(position-offset_pos[filenumber],x,'ro-',label="Scan Data",markersize=4)
...
plt.title(r"Peak input intensity: %.2f"%(I[filenumber]*10**(-16))+r"$\pm$%.2f"%(I_err[filenumber])+r" $TW/cm^2$")
plt.legend()
plt.show()
