...
a4_array=np.arange(5,10,0.1)*10**(-44)
...
for i in range(len(a4_array)):
    ...
    plt.grid()
    plt.xlabel("z position in mm"+"\n"+r"$\alpha_4: $ %f"%alpha4+r"$\times 10^{-34} cm^5W^{-3}$"+"        "+"absolute sq error: %f"%(absolute_error))
...
plt.plot(sample_pos,fit_y,'k-',label="Fit data")
plt.grid()
plt.xlabel("z position in mm"+"\n"+r"$\alpha_4: $ %f"%alpha4+r"$\times 10^{-34} cm^5W^{-3}$"+"        "+"absolute sq error: %f"%(min(chi_sq_list)))
plt.ylabel("Normalized transmission T(z)")
plt.title("4 Photon absorption by z-scan of ga2O3"+"\n"+filename_at_title)
plt.show()


plt.plot(a4_array*10**44,chi_sq_list,'ko-')
plt.plot(a4_array*10**44,chi_sq_list,'ro')
plt.title(r"Absolute error for the fitting for different choice of $\alpha_4$"+"\n"+"for I: %.2f"%(I[filenumber]/10**16)+r"$\mu J$")
plt.xlabel(r"$\alpha_4$  ($\times 10^{-44} cm^5/W^3$)",fontsize=12)#+"\n"+r"$calculated\ by: Abs\ Error=\sum_i\frac{(y_i^{analytical}-data_i^{measured})^2}{(data_i^{measured})^2}$",fontsize=12)
