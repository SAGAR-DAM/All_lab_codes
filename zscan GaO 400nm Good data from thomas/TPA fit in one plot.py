...
for i in range(len(files)):
    ...
    plt.plot(pos*10,fit_y,'k-',label="fit data")
    plt.xlabel(r"z position in mm"+"\n"+r"value from fit: $a_2$: %f"%a2+r"$^{+%.5f}$"%(ua2_err[i])+r"$_{-%.5f}$"%(la2_err[i])+" cm/GW")
...
for i in range(len(files)):
    ...
    plt.plot(pos*10,fit_y,'k-')
    plt.grid()
    plt.legend(fontsize=9)
    plt.xlabel("Z (mm)",fontsize=10)
    plt.ylabel("Normalized transmittance",fontsize=10)
