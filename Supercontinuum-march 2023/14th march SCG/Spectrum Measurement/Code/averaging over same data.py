...
def main():
    ...
    fileset=[5,6,5,4,4,4,5]      #number of files for which the  averaging is carried out
    
    rangearr=[450,900]           #range of the x axis data needed
    fileindex=0                  
    ...
    for i in range(len(fileset)):
        ...
        plt.savefig("D:\\data Lab\\Supercontinuum-march 2023\\14th march SCG\\Spectrum Measurement\\averaged over plots\\%s.jpg"%(h[2:len(h)-1]),bbox_inches='tight')
        plt.show()
    ...
    for i in range(len(labels)):
        ...
        plt.ylabel("Intensity (Arb unit)", fontname='Times New Roman')
        plt.title("Comparisn of different supercontinuums")
