...
def main():
    files=glob.glob("D:\\data Lab\\Supercontinuum-march 2023\\15th march SCG_set2\\Spectrum Measurement\\data\\*.trt")
    fileset=[5,5,10,5,3,3,5]      #number of files for which the  averaging is carried out
    ...
    for i in range(len(fileset)):
        ...
        plt.savefig("D:\\data Lab\\Supercontinuum-march 2023\\15th march SCG_set2\\Spectrum Measurement\\averaged over plots\\%s.jpg"%(h[2:len(h)-1]),bbox_inches='tight')
        plt.show()
        
    
    for i in range(len(labels)):
        plt.plot(wavelength,intensity_matrix[i],label=labels[i])
        plt.grid()
